    ...
    <div class="auth-page">
      <div class="auth-card">
        ...
        <form @submit.prevent="loginUser">
          ...
          <input type="password" id="password" v-model="password" placeholder="Password" autocomplete="current-password" required />
    ...
    async loginUser() {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          } else if (role === 'hr' || role === 'hr0' || role === 'bda' || role === 'employee') {
...
input { width:100%; padding:10px 12px; margin-top:6px; border-radius:8px; border:1px solid #e6e9ee }
.primary { width:100%; background:#2d8cff; color:#fff; border:none; padding:10px 14px; border-radius:8px; cursor:pointer; margin-top:20px }
